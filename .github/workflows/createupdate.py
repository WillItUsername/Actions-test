...
def check_format_and_prepare_notebook(notebook_path, env):
    ...
    temp_view_str = re.sub(r"dap_[dtp]_", f"dap_{env}_", temp_view_str)

    with open(notebook_path.replace('.sql', '_temp.sql'), 'w') as temp_notebook:
        temp_notebook.write(temp_view_str)

if __name__ == '__main__':
    check_format_and_prepare_notebook(sys.argv[1], sys.argv[2])
